...
const retrieveTargets = element => {
	if(element.hasAttribute(constants.TARGET_ALL))
		return document.querySelectorAll(element.getAttribute(constants.TARGET_ALL));
	else if(element.hasAttribute(constants.TARGET_PARENT))
		return element.parentElement.querySelectorAll(element.getAttribute(constants.TARGET_PARENT));
	else if(element.hasAttribute(constants.TARGET_SELF))
		return element.querySelectorAll(element.getAttribute(constants.TARGET_SELF));
	return [];
};
	...
	if (!target.closest('['+constants.TARGET_STATE+'="true"]')) {
		[...document.querySelectorAll('['+constants.CLASS+']['+constants.OUTSIDE+']')].forEach((element) => {
			if(element != target && element.isToggleActive)
				if(element.hasAttribute(constants.GROUP)) manageGroup(element);
				else manageToggle(element);
		});
		if(target.hasAttribute(constants.OUTSIDE) && target.isToggleActive)
			document.addEventListener(target.getAttribute(constants.EVENT) || 'click', documentEventHandler, false);
	}
};
...
const manageTriggerOutside = element => {
	if(element.hasAttribute(constants.OUTSIDE)) {
		if(element.hasAttribute(constants.GROUP))
			console.warn("You can't use '"+constants.OUTSIDE+"' on a grouped trigger");
		else {
			if(element.isToggleActive)
				document.addEventListener(element.getAttribute(constants.EVENT) || 'click', documentEventHandler, false);
			else
				document.removeEventListener(element.getAttribute(constants.EVENT) || 'click', documentEventHandler, false);
		}
	}
};
...
const manageTarget = (targetElement, triggerElement) => {
	if(triggerElement.hasAttribute(constants.OUTSIDE))
		targetElement.setAttribute(constants.TARGET_STATE, triggerElement.isToggleActive);
...
const manageToggle = element => {
	...
	if(!element.hasAttribute(constants.TARGET_ONLY))
...
const manageActiveByDefault = element => {
	...
	if(!element.hasAttribute(constants.TARGET_ONLY) && !element.classList.contains(className))
	...
	let targetElements = retrieveTargets(element);
	for(var i=0;i<targetElements.length;i++) {
		if(!targetElements[i].classList.contains(element.getAttribute(constants.CLASS)))
...
const init = () => {
	...
	[...document.querySelectorAll('['+constants.CLASS+']['+constants.IS_ACTIVE+']')].forEach((trigger) => {
		if(trigger.hasAttribute(constants.GROUP)) {
			...
			if(retrieveGroupState(group).length > 0)
				console.warn("Toggle group '"+group+"' must not have more than one trigger with '"+constants.IS_ACTIVE+"'");
			else
	...
	[...document.querySelectorAll('['+constants.CLASS+']')].forEach((trigger) => {
		trigger.addEventListener(trigger.getAttribute(constants.EVENT) || 'click', (event) => {
			...
			if(trigger.hasAttribute(constants.GROUP)) manageGroup(trigger);
			...
			if(isEscape) {
				triggerEscElements.forEach((trigger) => {
					if(trigger.isToggleActive) {
						if(trigger.hasAttribute(constants.GROUP))
							console.warn("You can't use '"+constants.ESCAPE+"' on a grouped trigger");
						else manageToggle(trigger);
					}
				});
			}
